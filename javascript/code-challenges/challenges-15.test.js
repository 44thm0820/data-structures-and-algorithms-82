...
let biggerThanLuke = (arr) => {
  ...
  const massLuke = Number(arr.find(obj => obj.name === 'Luke Skywalker').mass);
  return arr.filter(obj => Number(obj.mass) > massLuke).map(obj2 => obj2.name).join(' - ');
};
...
const detectTicTacToeWin = (board) => {
  ...
  return arrPossibleWays.includes(winningTrio[0]) ||
    arrPossibleWays.includes(winningTrio[1]);
